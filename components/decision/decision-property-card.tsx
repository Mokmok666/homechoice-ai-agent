    ...
    <Link href={`/results/${property.id}`} className="card flex h-full min-w-0 flex-col p-5 transition hover:-translate-y-0.5 hover:shadow-[0_14px_38px_rgba(58,55,46,0.08)]">
      ...
      <div className="mt-5 flex min-w-0 items-start gap-4">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-2 flex items-start gap-1.5 text-sm text-[#7d807a]"><MapPin size={14} className="mt-0.5 shrink-0" />{property.city} · {property.district}</p>
          <p className="mt-2 text-[15px] text-[#696c67]">{property.layout} · {property.area}㎡ · {property.floor}</p>
        </div>
      </div>

      <div className="mt-5 grid grid-cols-[minmax(0,1fr)_76px] items-end gap-4 border-t border-[#eceae5] pt-5">
        <div className="min-w-0">
          <p className="text-[13px] text-[#92938f]">预期成交价</p>
          ...
          {property.listingPrice && <p className="mt-1 text-[13px] text-[#8d8f89]">挂牌价 {property.listingPrice} 万</p>}
        </div>
        {result.overallScore === null ? (
          <div className="grid size-[76px] place-items-center rounded-full border border-dashed border-[#d9d8d2] text-center text-xs leading-4 text-[#8a8c87]">阶段性<br />判断</div>
        ) : (
          <ScoreRing score={result.overallScore} size={76} label="当前匹配" />
        )}
      </div>

      <div className="mt-5 grid grid-cols-2 gap-3 rounded-xl bg-[#f7f6f2] p-3 text-[13px]">
      ...
      <div className="mt-3 grid grid-cols-3 gap-2 text-xs leading-5 text-[#777a74]">
      ...
      <div className="mt-4 space-y-1.5 text-sm leading-6 text-[#747772]">
