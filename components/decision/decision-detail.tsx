...
export function DecisionDetail({ propertyId }: { propertyId: string }) {
  ...
  const aiEvidence = result.confidence.evidenceItems.filter((item) => item.category === "ai_inferred");
    ...
    <main className="page">
      ...
      <section className="card mt-5 p-6 sm:p-8">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-3">
          ...
          <div className="rounded-xl bg-[#f7f4ed] p-5"><h3 className="font-semibold text-[#806b3e]">AI分析项（{result.confidence.aiAnalysisProgress.completed}/{result.confidence.aiAnalysisProgress.total}）</h3><ul className="mt-3 space-y-3 text-xs leading-5 text-[#746b59]">{aiEvidence.map((item) => <li key={item.id}><b>○ {item.title}</b><span className="mt-0.5 block text-[#8a806d]">{item.description}</span></li>)}</ul></div>
