    ...
    <main className="page">
      ...
      <section className="card mt-6 p-6 sm:p-7">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-3 text-[15px] text-[#727570]">{property.city} · {property.district} · {property.address}</p>
            <p className="mt-3 text-[15px] text-[#555]">{property.area}㎡ · {property.layout} · {property.floor} · 预期成交价 {property.totalPrice} 万{property.listingPrice ? ` · 挂牌价 ${property.listingPrice} 万` : ""}</p>
          </div>
        ...
        <div className="mt-7 grid gap-4 border-t border-[#eceae5] pt-6 sm:grid-cols-3">
          <div className="rounded-xl bg-[#f7f6f2] p-4"><span className="text-[13px] text-[#92938f]">数据完整度</span><b className="mt-2 block text-xl">{result.confidence.dataCompletenessPercent}%</b><small className="mt-2 block text-[13px] leading-5 text-[#858782]">基础 {completeness.sections.basic}/60 · 生活 {completeness.sections.living}/20 · 决策 {completeness.sections.decision}/20</small></div>
          <div className="rounded-xl bg-[#f7f6f2] p-4"><span className="text-[13px] text-[#92938f]">分析可信度</span><b className="mt-2 block text-xl">{ANALYSIS_CONFIDENCE_LABELS[result.confidence.analysisConfidence]}</b></div>
          <div className="rounded-xl bg-[#f7f6f2] p-4"><span className="text-[13px] text-[#92938f]">分析日期</span><b className="mt-2 block text-xl">{state.asOfDate}</b></div>
        </div>
      </section>
      ...
      <section className="card mt-5 p-6 sm:p-7">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-3">
          <div className="rounded-xl bg-[#f5f8f3] p-5"><h3 className="text-base font-semibold text-[#607158]">已确认信息</h3><ul className="mt-3 space-y-3 text-sm leading-6 text-[#686b66]">{confirmedEvidence.map((item) => <li key={item.id}><b>✓ {item.title}</b><span className="mt-0.5 block text-[#858782]">{item.description}</span></li>)}</ul></div>
          <div className="rounded-xl bg-[#f7f4ed] p-5"><h3 className="text-base font-semibold text-[#806b3e]">外部证据覆盖（{coverage.completed}/{coverage.total}）</h3><ul className="mt-3 space-y-3 text-sm leading-6 text-[#746b59]">{coveredDimensions.map((item) => <li key={item.dimensionKey}><b>○ {DIMENSION_LABELS[item.dimensionKey]}</b><span className="mt-0.5 block text-[#8a806d]">{item.status === "verified" ? "已有可靠公开来源支持" : "已有公开来源，仍需交叉核验"}</span></li>)}</ul>{coverage.uncovered.length > 0 && <p className="mt-3 text-sm leading-6 text-[#8a806d]">尚未覆盖：{coverage.uncovered.map((key) => DIMENSION_LABELS[key]).join("、")}</p>}</div>
          <div className="rounded-xl bg-[#f7f6f2] p-5"><h3 className="text-base font-semibold">可进一步确认</h3><ul className="mt-3 space-y-3 text-sm leading-6 text-[#686b66]">{optionalEvidence.map((item) => <li key={item.id}><b>△ {item.title}</b><span className="mt-0.5 block text-[#858782]">{item.description}</span></li>)}</ul></div>
        </div>
      </section>

      <section className="card mt-5 p-6 sm:p-7">
        <div className="flex items-center justify-between gap-4"><div><h2 className="font-serif text-2xl">15 维证据明细</h2><p className="mt-2 text-sm leading-6 text-[#83857f]">分数仅在证据足够时显示；权重来自购房目的与 Top 3 优先级。</p></div><Database className="text-[#75886d]" /></div>
        <div className="mt-6 grid gap-3 lg:grid-cols-2">
          {result.dimensions.map((dimension) => {
            ...
            return <article key={dimension.key} className="rounded-xl border border-[#e8e6e0] p-4">
              <div className="flex items-center justify-between gap-4"><div><h3 className="text-base font-medium">{DIMENSION_LABELS[dimension.key]}</h3><p className="mt-1 text-[13px] text-[#93948f]">{dimension.key === "value_preservation" ? "结构派生" : dimension.evidence.some((item) => item.source === "amap") ? "高德地图证据" : dimension.evidence.some((item) => item.source === "web") ? "公开来源证据" : DIMENSION_TYPE_LABELS[DIMENSION_TYPES[dimension.key]]} · 最终权重 {dimension.finalWeight.toFixed(1)}%</p></div><div className="text-right"><b className="text-xl text-[#617359]">{dimension.score ?? "—"}</b><span className="ml-2 rounded-full bg-[#f1f0eb] px-2 py-1 text-xs text-[#737570]">{dimensionStatusLabel(dimension, webDimension?.status)}</span></div></div>
              {dimension.score !== null && <div className="progress mt-3"><span style={{ width: `${dimension.score}%` }} /></div>}
              {dimension.evidence.length > 0 && <ul className="mt-3 space-y-1 text-sm leading-6 text-[#6f726d]">{dimension.evidence.map((item) => <li key={`${item.source}-${item.description}`} className="flex gap-2"><CheckCircle2 size={13} className="mt-1 shrink-0 text-[#75886d]" />{item.description}</li>)}</ul>}
              {hasWebEvidence && webDimension && <details className="mt-3 text-sm text-[#617359]"><summary className="w-fit cursor-pointer select-none font-medium">查看来源 →</summary><ul className="mt-2 space-y-2 rounded-lg bg-[#f7f8f5] p-3 text-[#6f726d]">{webDimension.facts.slice(0, 2).map((fact) => <li key={fact.sourceUrl} className="leading-6"><a href={fact.sourceUrl} target="_blank" rel="noreferrer" className="font-medium text-[#617359] underline decoration-[#bdc7b8] underline-offset-2">{conciseSourceTitle(fact.sourceTitle)}</a><span className="block text-xs text-[#92948f]">{fact.sourceDomain ?? "公开来源"}{fact.publishedAt ? ` · ${fact.publishedAt.slice(0, 10)}` : ""}</span></li>)}</ul></details>}
              {!hasWebEvidence && isWebTarget && !(dimension.status === "known" && dimension.missingInputs.length === 0) && <p className="mt-3 flex items-start gap-2 text-sm leading-6 text-[#8a7046]"><HelpCircle size={13} className="mt-1 shrink-0" />当前暂无可用公开来源，仍需进一步核验。</p>}
              {!isWebTarget && dimension.missingInputs.length > 0 && <p className="mt-3 flex items-start gap-2 text-sm leading-6 text-[#8a7046]"><HelpCircle size={13} className="mt-1 shrink-0" />{dimension.key === "value_preservation" || dimension.evidence.some((item) => item.source === "amap") || DIMENSION_TYPES[dimension.key] !== "ai" ? "可进一步确认" : "AI分析"}：{dimension.missingInputs.join("、")}</p>}
            </article>;
          })}
        </div>
      </section>
      ...
      <section className="card mt-5 p-6 sm:p-7">
        ...
        <p className="mt-2 text-sm leading-6 text-[#777a74]">当前推荐已经生成；可进一步确认的信息只影响可信度，不是继续分析的前置条件。</p>
