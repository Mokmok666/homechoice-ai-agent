...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  if (typeof input.city !== "string" || typeof input.district !== "string" || typeof input.keyword !== "string") {
    return NextResponse.json({ ok: false, error: { code: "INVALID_INPUT", message: "city、district 和 keyword 必须是字符串。" } }, { status: 400 });
  }
  try {
    const candidates = await searchPropertyLocations({ city: input.city, district: input.district, keyword: input.keyword });
    ...
    return NextResponse.json({ ok: false, error: { code: "AMAP_API_ERROR", message: "高德位置搜索发生未知错误。" } }, { status: 502 });
  }
}
